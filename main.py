...
GOLD = (255, 215, 0)
PANEL_COLOR = (10, 10, 20, 200)  # полупрозрачная тёмная плашка
...
karma = 0
ending_type = ""
has_wolf_fang = False  # артефакт от волчонка
has_glow_mushroom = False  # артефакт от грибов
...
def draw_panel(x, y, w, h, alpha=200):
    """Рисует полупрозрачную тёмную плашку со скруглением."""
...
def draw_text_with_panel(text, font_obj, color, y_top, padding=25):
    """
    Рисует текст в полупрозрачной плашке сверху экрана.
    Автоматически переносит строки и центрирует по горизонтали.
    """
    # Разбиваем на строки
    words = text.split(' ')
...
while running:
    ...
    # --- СЦЕНА 4: ПЕЩЕРА (ДУХ) ---
        ...
        def chase_spirit():
            ...
            # Хороший финал, если есть артефакты ИЛИ положительная карма
